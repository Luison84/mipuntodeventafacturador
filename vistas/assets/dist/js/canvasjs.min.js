/* 
 CanvasJS Chart - v3.7.25 GA - https://canvasjs.com/ 
...
b);if(u.indexLabelFormatter||b.indexLabelFormatter)z={chart:A.options,dataSeries:u,dataPoint:b,total:v.total,percent:v.percent};b=b.indexLabelFormatter?b.indexLabelFormatter(z):b.indexLabel?A.replaceKeywordsWithValue(b.indexLabel,b,u,D):u.indexLabelFormatter?u.indexLabelFormatter(z):u.indexLabel?A.replaceKeywordsWithValue(u.indexLabel,b,u,D):b.label?b.label:"";0>=m&&(m=0);1E3>t&&1E3-t<r&&(t+=1E3-t);P.roundRect||Da(P);c=new ja(P,{fontSize:f,fontFamily:k,fontColor:e,fontStyle:l,fontWeight:h,horizontalAlign:c,
...
0>=F&&(F=0)):"pyramid"===u.type&&(F=Q=0,u.reversed=u.reversed?!1:!0);var B=S+a/2,T=S,V=S+a,U=u.reversed?N:$,O=B-F/2,W=B+F/2,Ca=u.reversed?$+Q:N-Q,ta=u.reversed?$:N;a=[];var B=[],R=[],G=[],Y=$,ca,aa=(Ca-U)/(O-T),fa=-aa,S="area"===(u.valueRepresents?u.valueRepresents:"height")?c():d();if(-1!==S){if(u.reversed)for(G.push(Y),F=S.length-1;0<F;F--)Y+=S[F],G.push(Y);else for(F=0;F<S.length;F++)Y+=S[F],G.push(Y);if(u.reversed)for(F=0;F<S.length;F++)G[F]<Ca?(a.push(O),B.push(W),ca=F):(a.push((G[F]-U+aa*T)/
aa),B.push((G[F]-U+fa*V)/fa));else for(F=0;F<S.length;F++)G[F]<Ca?(a.push((G[F]-U+aa*T)/aa),B.push((G[F]-U+fa*V)/fa),ca=F):(a.push(O),B.push(W));for(F=0;F<S.length-1;F++)Y=u.reversed?y[y.length-1-F].color?y[y.length-1-F].color:u._colorSet[(y.length-1-F)%u._colorSet.length]:y[F].color?y[F].color:u._colorSet[F%u._colorSet.length],F===ca?R.push({x1:a[F],y1:G[F],x2:B[F],y2:G[F],x3:W,y3:Ca,x4:B[F+1],y4:G[F+1],x5:a[F+1],y5:G[F+1],x6:O,y6:Ca,id:F,height:G[F+1]-G[F],color:Y}):R.push({x1:a[F],y1:G[F],x2:B[F],
...
-1,0):A.animationEnabled&&!A.animatedRender&&n(A,-1,0);this.funnelPyramidClickHandler=function(a){var b=-1;if(!ha&&!A.isAnimating&&(s(a.dataSeries.explodeOnClick)||a.dataSeries.explodeOnClick)&&(b=u.reversed?y.length-1-a.dataPointIndex:a.dataPointIndex,0<=b)){a=b;if("funnel"===u.type||"pyramid"===u.type)u.reversed?y[y.length-1-a].exploded=y[y.length-1-a].exploded?!1:!0:y[a].exploded=y[a].exploded?!1:!0;n(A,b,500)}};return{source:P,dest:this.plotArea.ctx,animationCallback:function(a,b){M.fadeInAnimation(a,
...
this.ctx.lineWidth=f/8;this.ctx.beginPath();var w=g-0.08*this.lineHeight+0.1*this.lineHeight,u=r+0.15*this.lineHeight,v=0.7*this.lineHeight,x=v+0.02*this.lineHeight;this.ctx.moveTo(w,u);this.ctx.lineTo(w+v,u);this.ctx.stroke();this.ctx.beginPath();this.ctx.moveTo(w+v/2,u);this.ctx.lineTo(w+v/2,u+x);this.ctx.stroke();this.ctx.beginPath();this.ctx.moveTo(w,u+x);this.ctx.lineTo(w+v,u+x);this.ctx.stroke();this.errorMarkerColor.shift()}W.drawMarker(g+f/2,r+this.lineHeight/2,this.ctx,q.markerType,"error"===
...
g;this.ghostCtx.beginPath();this.ghostCtx.fillRect(s,q.textBlock.y-this.lineHeight/2,q.textBlock.x+q.textBlock.width-s,q.textBlock.height);q.x1=this.chart._eventManager.objectMap[q.id].x1=s;q.y1=this.chart._eventManager.objectMap[q.id].y1=q.textBlock.y-this.lineHeight/2;q.x2=this.chart._eventManager.objectMap[q.id].x2=q.textBlock.x+q.textBlock.width;q.y2=this.chart._eventManager.objectMap[q.id].y2=q.textBlock.y+q.textBlock.height-this.lineHeight/2}h+=m}};oa(Q,G);Q.prototype.getDefaultAxisPlacement=
...
(k.textBlock.height-h.textBlock.fontSize/2)*Math.sin(Math.PI/180*Math.abs(k.textBlock.angle)),A=h.textBlock.width*Math.cos(Math.PI/180*Math.abs(h.textBlock.angle))+(h.textBlock.height-h.textBlock.fontSize/2)*Math.sin(Math.PI/180*Math.abs(h.textBlock.angle)));if(m.panEnabled)for(g=0;g<c.length;g++)p[g]=s(m.sessionVariables.axisY.height)?m.sessionVariables.axisY.height=p[g]:m.sessionVariables.axisY.height;else for(g=0;g<c.length;g++)m.sessionVariables.axisY.height=p[g];if(c&&0<c.length)for(g=c.length-
...
"cross"===b&&(c.strokeStyle=f,c.lineWidth=e/4,c.beginPath(),c.moveTo(a-e/2,d-e/2),c.lineTo(a+e/2,d+e/2),c.stroke(),c.moveTo(a+e/2,d-e/2),c.lineTo(a-e/2,d+e/2),c.stroke())}},drawMarkers:function(a){for(var d=0;d<a.length;d++){var c=a[d];W.drawMarker(c.x,c.y,c.ctx,c.type,c.size,c.color,c.borderColor,c.borderThickness)}}};return n}();B.version="v3.7.25 GA";window.CanvasJS&&(B&&!window.CanvasJS.Chart)&&(window.CanvasJS.Chart=B)})();
